...
# from record import save_text_response,save_voice_response
                            ...
                            if item.get('type') == 'message' and 'content' in item:
                                for content in item['content']:
                                    if content.get('type') == 'audio' and 'transcript' in content:
                                        ...
                                        # Saving the text ai response
                                        # await save_text_response(response_content)
                                        ...
                                        if "[HANGUP_CALL]" in response_content:
                                            ...
                                            if should_hang_up:
                                                print("Waiting for 10 seconds before hanging up...")
                                                time.sleep(10)
                                                print("Hanging up the call now.")

                    if response.get('type') == 'response.audio.delta' and 'delta' in response:
                        audio_payload = base64.b64encode(base64.b64decode(response['delta'])).decode('utf-8')
                        print("Audio data :" ,audio_payload)
                        
                        # Saving the audio file
                        # await save_voice_response(audio_payload)
                        audio_delta = {
                            "event": "media",
